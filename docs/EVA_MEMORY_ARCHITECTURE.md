...
-- PRAGMA user_version = 2;  -- v2 (Phase 14): is_seeded on graph_nodes/graph_edges
...
```json
{
  ...
  "identity": {
    ...
    "provenance": {
      "stated_self": {"evidence": ["<entry-uuid>"], "source": "model", "last_seen": "2026-06-10"},
      "principles":  {"evidence": ["<entry-uuid>"], "source": "model", "last_seen": "2026-06-10"}
    }
  },
  "goals": [
    {
  ...
  "emotional_baseline": {
    ...
    "provenance": {
      "typical_mood":   {"source": "code",  "evidence": ["<entry-uuid-1>", "<entry-uuid-3>"]},
      "known_triggers": {"source": "model", "evidence": ["<entry-uuid-3>"], "last_seen": "2026-06-08"},
      "what_helps":     {"source": "model", "evidence": ["<entry-uuid-4>"], "last_seen": "2026-06-08"}
    }
  },
  "open_loops": [
    {
...
**Field-keyed provenance (v2).** `identity` and `emotional_baseline` are singleton dicts with no claim `id`, so each inferred field carries its own evidence in a `provenance` object keyed by field name (`stated_self`, `principles`, `known_triggers`, `what_helps`, `typical_mood`). Every field Eva infers is grounded in real entries — the same evidence discipline as the list claims. `typical_mood` is **code-derived** (the rounded mean of L1 mood history) and carries `"source": "code"`; the model never authors it. The other fields carry `"source": "model"` (or `"user"` once corrected).

**Anchors** are the identifiers of things the user manually corrected in `profile.md`. For list claims (goals, patterns, loops) that is the claim `id`; for a singleton identity/baseline field it is a synthetic path — `identity.stated_self`, `identity.principles`, `baseline.known_triggers`, `baseline.what_helps`, `baseline.typical_mood`. All carry `"source": "user"` on the field/claim. The operation grammar cannot `weaken`/`strengthen` an anchored claim or overwrite an anchored field — only the user can change them via `PUT /profile`, and a rebuild preserves them.

**`profile.md` ↔ `profile.json` sync:** `profile.md` is a human-readable rendering of `profile.json`, regenerated by `profile.py` after each update. When the user edits and saves `profile.md`, a `PUT /profile` call re-parses the Markdown into a diff of JSON operations and applies them as `set_anchor` ops. The parse is lenient: unparseable sections are left unchanged and the user is warned (an out-of-range typical_mood is rejected, not stored). The `anchors` list tracks what the user corrected — claim IDs for list claims, and synthetic `section.field` paths for the singleton identity/baseline fields.
...
| `weaken` | `claim_id`, `reason`, `evidence[]` | confidence -= 0.15 (floor 0.0); flag for review if < 0.2 |
...
| `set_identity` | `text`, `evidence[]` | Replace `identity.stated_self` (rejected if anchored) |
| `add_principle` | `text`, `evidence[]` | Append to `identity.principles`, deduped (rejected if anchored) |
| `add_baseline_item` | `field`, `text`, `evidence[]` | Append to `emotional_baseline.known_triggers` or `what_helps`, deduped (rejected if anchored) |
| `set_anchor` | `claim_id` | Mark as user-corrected; blocks model overwrite (user-only op) |

**`typical_mood` has no verb.** It is computed by code as the rounded mean of L1 mood history and written on rebuild/consolidation; the model may never emit or influence it (its provenance carries `"source": "code"`).
